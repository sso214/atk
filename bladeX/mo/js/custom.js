$(document).ready(function(){
    ...
    $(".lang-list li").on('click', function(){
        ...
        getLangSrc($('.pop.invalid-coupon img'));
        getLangSrc($('.pop.server-unavailable img'));
        getLangSrc($('.pop.market-unavailable img'));
        getLangSrc($('.pop.same-coupone-over-count img'));
        getLangSrc($('.pop.db-error img'));
    ...
    $('.input-box button').on('click', function () {
        ...
        // if (쿠폰 정보가 잠롯됐을 시) return popOpenEvt('invalid-coupon');
        // if (쿠폰 사용 가능한 서버가 아닐 시) return popOpenEvt('server-unavailable');
        // if (쿠폰 사용 가능한 마켓이 아닐 시) return popOpenEvt('market-unavailable');
        // if (같은 종류의 쿠폰 사용 횟수를 초과했을 시) return popOpenEvt('same-coupone-over-count');
        // if (쿠폰 사용 중 DB 오류가 발생했을 시) return popOpenEvt('db-error');
        popOpenEvt('success');
    });
